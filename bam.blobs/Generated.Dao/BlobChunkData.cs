...
namespace Bam.Blobs.Data.Local.Dao
{
	...
	[Serializable]
	[Bam.Data.Table("BlobChunkData", "LocalBlobData")]
	public partial class BlobChunkData: Bam.Data.Dao
	{
		public BlobChunkData():base()
		{
			this.SetKeyColumnName();
			this.SetChildren();
		}

		public BlobChunkData(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public BlobChunkData(IDatabase db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public BlobChunkData(IDatabase db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		[Bam.Exclude]
		public static implicit operator BlobChunkData(DataRow data)
		{
			return new BlobChunkData(data);
		}
    ...
    // property:BlobIndex, columnName: BlobIndex	
    [Bam.Data.Column(Name="BlobIndex", DbDataType="BigInt", MaxLength="19", AllowNull=true)]
    public long? BlobIndex
    {
        get
        {
            return GetLongValue("BlobIndex");
        }
        set
        {
    ...
    public DateTime? Created
    {
		...
		public override IQueryFilter GetUniqueFilter()
		{
			...
			{
				var colFilter = new BlobChunkDataColumns();
				return (colFilter.KeyColumn == GetDbId());
			}
		}

		/// <summary>
        /// Return every record in the BlobChunkData table.
        /// </summary>
		/// <param name="database">
		...
		public static BlobChunkDataCollection LoadAll(IDatabase database = null)
		{
            ...
            sql.Select<BlobChunkData>();
            var results = new BlobChunkDataCollection(db, sql.ExecuteGetDataTable(db))
            {
                Database = db
            };
            return results;
        ...
        public static async Task BatchAll(int batchSize, Action<IEnumerable<BlobChunkData>> batchProcessor, IDatabase database = null)
		{
			...
			{
				BlobChunkDataColumns columns = new BlobChunkDataColumns();
				var orderBy = Bam.Data.Order.By<BlobChunkDataColumns>(c => c.KeyColumn, Bam.Data.SortOrder.Ascending);
		...
		/// </summary>
		/// <param name="where">A Func delegate that recieves a BlobChunkDataColumns
		...
		/// Execute a query and return the results.
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a BlobChunkDataColumns
		...
		/// WhereDelegate`BlobChunkDataColumns`.
		/// </summary>
		...
		public static BlobChunkDataCollection Top(int count, WhereDelegate<BlobChunkDataColumns> where, OrderBy<BlobChunkDataColumns> orderBy, IDatabase database = null)
		{
			...
			query.Top<BlobChunkData>(count);
			query.Where(filter);

			if(orderBy != null)
			{
				query.OrderBy<BlobChunkDataColumns>(orderBy);
			}

			query.Execute(db);
			var results = query.Results.As<BlobChunkDataCollection>(0);
			results.Database = db;
			return results;
		}
		...
		private static BlobChunkData OneOrThrow(BlobChunkDataCollection c)
		{
			if(c.Count == 1)
			{
				return c[0];
